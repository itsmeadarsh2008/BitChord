...
object Mp4Tagger {
    ...
    private fun parseBoxes(bytes: ByteArray, start: Int, end: Int): List<BoxRef> {
        ...
        while (pos + 8 <= end) {
            ...
            val type = bytes.decodeLatin1(pos + 4, 4)
            var headerLen = 8
            var size = size32
            if (size32 == 1L) {
                if (pos + 16 > end) break
    ...
    private fun box(type: String, payload: ByteArray): ByteArray {
        ...
        type.encodeLatin1().copyInto(out, 4)
    ...
    private fun textItem(fourCc: String, text: String): ByteArray =
        box(fourCc, dataAtom(1, text.encodeUtf8()))
    ...
    private fun freeformItem(name: String, text: String): ByteArray {
        ...
        val mean = box("mean", ByteArray(4) + MEAN.encodeUtf8())
        val label = box("name", ByteArray(4) + name.encodeUtf8())
        return box("----", mean + label + dataAtom(1, text.encodeUtf8()))
    }
    ...
    private fun hdlrAtom(): ByteArray {
        ...
        "mdir".encodeLatin1().copyInto(body, 8)
