...
object LastFM {
    ...
    fun normalizeEndpoint(endpoint: String): String {
        ...
        require(url.protocol == URLProtocol.HTTP || url.protocol == URLProtocol.HTTPS) {
            "Last.fm endpoint must be http(s): $endpoint"
        }
        require(url.host.isNotBlank()) { "Last.fm endpoint needs a host: $endpoint" }
        require(url.encodedQuery.isEmpty() && url.fragment.isEmpty()) {
            "Last.fm endpoint takes no query or fragment: $endpoint"
        }
        val path = url.encodedPath.let { if (it.isBlank() || it == "/") "/2.0" else it.trimEnd('/') }
        return buildString {
            append(url.protocol.name.lowercase())
            append("://")
            append(url.host)
            if (url.port != url.protocol.defaultPort) append(":${url.port}")
            append(path)
            append("/")
        }
    }
    ...
    private suspend fun postAndRead(
        ...
        val response = try {
            ...
            throw LastFmException(e.response.status.value, e.response.status.description)
        }
